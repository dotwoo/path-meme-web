document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadBlogPost(postId) {
        try {
            const response = await fetch(`https://min-blog.dotwoo.workers.dev/api/blog-posts/${postId}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    async function loadBlogPostsByTag(tagName, timeline) {
        try {
            const response = await fetch(`https://min-blog.dotwoo.workers.dev/api/blog-posts?tag=${encodeURIComponent(tagName)}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
